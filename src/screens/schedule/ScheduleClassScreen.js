...
const ScheduleClassScreen = () => {
  ...
  const [allTimes] = useState(['08:00', '09:00', '10:00', '11:00', '14:00', '15:00', '16:00', '17:00']);
  ...
  const fetchUnavailableTimes = useCallback(async (date) => {
    try {
      const response = await fetch(`${API_URL}/api/classes/unavailable-times/${date}`);
  ...
  const programarClase = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result.error || 'No se pudo programar la clase.');
